...
from .planner import CloseSubtask, CloseSubtaskConfig, TaskPlan
...
class CloseSubtaskTrainEnv(SubtaskTrainEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        tp0 = task_plans[0]
        assert len(tp0.subtasks) == 1 and isinstance(tp0.subtasks[0], CloseSubtask), (
    ...
    def _load_scene(self, options):
        self.closed_with_ee_near_handle = torch.zeros(
    ...
    def _initialize_episode(self, env_idx, options):
        ...
        self.link: Link = self.articulation.links[
            self.close_subtask.articulation_handle_link_idx
        ]
        self.qmax = self.articulation.qlimits[
            :, self.close_subtask.articulation_handle_active_joint_idx, 1
        ]
        self.qmin = self.articulation.qlimits[
            :, self.close_subtask.articulation_handle_active_joint_idx, 0
        ]
        self.target_qpos = (
            self.qmax - self.qmin
        ) * self.close_cfg.joint_qpos_close_thresh_frac + self.qmin
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        with torch.device(self.device):
            ...
            articulation_joint_qpos = self.articulation.qpos[
                :, self.close_subtask.articulation_handle_active_joint_idx
            ]
            frac_to_close_left = torch.clamp(
                (articulation_joint_qpos - self.target_qpos)
                / (self.qmax - self.target_qpos),
                min=0,
            )
            ...
            self.closed_with_ee_near_handle = is_closed & (
                self.closed_with_ee_near_handle
                | (tcp_to_handle_dist <= self.closed_with_ee_near_handle_thresh)
            )
            ...
            slow_close_reward = (
                5 * (torch.abs(articulation_joint_qvel) <= self.ideal_qvel).float()
            )

            # when articulation close, return to rest
            arm_torso_resting_orientation_reward = 1 - (
                (
                    torch.tanh(arm_torso_to_resting_diff)
                    + torch.tanh(arm_torso_to_resting_diff / 5)
                )
                / 2
            )
            ...
            ee_rest_reward = 2 - (
                torch.tanh(ee_to_rest_dist) + torch.tanh(3 * ee_to_rest_dist)
            )
