...
def make_mshab_example() -> dict:
    ...
    return {
        "observation/state": np.random.rand(42),  # 完整 state，会在 MshabInputs 里提取 tcp_pose
...
class MshabInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # 只取 tcp_pose(7) 部分，索引 24:31
        # state 结构: qpos(12) + qvel(12) + tcp_pose(7) + obj_pose(7) + goal_pos(3) + is_grasped(1) = 42
        state = data["observation/state"]
        tcp_pose = state[..., 24:31]  # 提取 tcp_pose (7维)
        inputs = {
            "state": tcp_pose,
            "image": {
